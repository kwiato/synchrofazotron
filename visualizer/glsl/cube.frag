// Synchrofazotron glsl preset: cube — a rotating cube raymarched from an SDF,
// bass drives the spin and the pulse. Audio uniforms come from
...
float map(vec3 p) {
    p.xz = rot(u_time * 0.6 + u_bass * 2.0) * p.xz;
    p.xy = rot(u_time * 0.45) * p.xy;
